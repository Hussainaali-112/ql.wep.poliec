...
function requestFullSync() {
  broadcastUpdate('requestFullSync', {});
}
...
function broadcastUpdate(type, data) {
  if (socket && socket.readyState === WebSocket.OPEN) {
    ...
    console.warn('WebSocket is not open. Update not sent:', { type, data });
  }
}
...
function handleUpdate(type, data) {
  switch (type) {
    case 'fullSync':
      ...
      break;
    case 'newCitizen':
      if (!citizens.some(c => c.id === data.id)) {
        citizens.push(data);
        renderCitizens();
      }
      break;
...
function saveCitizen() {
  ...
  if (citizen) {
    ...
    citizen.age = document.getElementById('age').value;
    citizen.gender = document.getElementById('gender').value;
    citizen.nationality = document.getElementById('nationality').value;
    citizen.job = document.getElementById('jobs').value;
    citizen.photo = document.getElementById('personPhoto').src;
    citizen.wanted = document.getElementById('wantedStatusBtn').textContent === 'Remove Wanted Status';
...
// Call initializeWebSocket when the page loads
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (message) => {
    try {
      ...
      console.log(`Received ${type} from ${ip}:`, data);

      switch (type) {
        case 'requestFullSync':
          ws.send(JSON.stringify({ type: 'fullSync', data: { citizens } }));
          break;
        case 'newCitizen':
          citizens.push(data);
...
// Save all citizens to localStorage
function saveToLocalStorage() {
    localStorage.setItem('citizens', JSON.stringify(citizens));
}
...
function addCitizen(citizen) {
    citizen.id = Date.now(); // Generate unique ID
    citizens.push(citizen);
    saveToLocalStorage();
}

// Edit a citizen
function updateCitizen(id, updatedCitizen) {
    const index = citizens.findIndex(c => c.id === id);
    if (index !== -1) {
        citizens[index] = { ...citizens[index], ...updatedCitizen };
        saveToLocalStorage();
    }
}
...
function exportDatabase() {
    const dataStr = JSON.stringify(citizens);
